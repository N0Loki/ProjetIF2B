 ...
 * @param joueur La couleur de la pièce
 * @return Une chaîne représentant le symbole Unicode
 */
const char* get_unicode_piece(TypePiece piece, Couleurs joueur) {
    if (joueur == BLANC) {
        switch (piece) {
            case ROI: return "♔";
            case DAME: return "♕";
...
/**
 * Fonction calculant et affichant les scores des deux joueurs et le gagnant
 *
...
void scorer(int taille, Case **board) {
    ...
    for (int i = 0; i < taille; i++) {
        for (int j = 0; j < taille; j++) {
            if (board[i][j].captured != NONE) {
                if (board[i][j].captured == BLANC) {
                    scoreBlanc++;
                } else if (board[i][j].captured == NOIR) {
                    scoreNoir++;
                }
            }
        }
    ...
    int highscore = scoreNoir;
    int winner = NOIR;
    if (scoreBlanc > scoreNoir) {
        highscore = scoreBlanc, winner = BLANC;
    }
    else if (scoreNoir == scoreBlanc) {
        winner = NONE;
        printf("\tEgalité, vous avez tout deux %d, il n'y a pas de gagnant !\n", highscore);
    }
    if (winner != NONE) {
        printf("\nFélicitations joueur %s, vous avez gagnez avec %d points !\n", winner == BLANC ? "Blanc" : "Noir", highscore);
    }
}
...
int gameTurn(int joueur, int taille, int modeJeu, int totalPieces[3][7], Case **board) {
    ...
    while (1) {
        ...
        printf("\t│[2] - Abandonner/Quitter la partie             │\n");
