...
void createSave(int joueur, int taille, int modeJeu, int totalPieces[3][7], Case **board, char fileName[]) {
    ...
    for (int i = 0; i < taille; i++) {
        for (int j = 0; j < taille; j++) {
            Case c = board[i][j];
            fprintf(f, "%d %d %d %d ", c.contenu, c.captureeParPiece, c.couleurPiece, c.captured);
        }
        fprintf(f, "\n");
    }
...
Case** loadSave(int* joueur, int* taille, int* modeJeu, int totalPieces[3][7], const char* filename) {
    ...
    for (int i = 0; i < *taille; i++) {
        ...
        for (int j = 0; j < *taille; j++) {
            int contenu, couleur, captured, captureeParPiece;
            fscanf(file, "%d %d %d %d", &contenu, &captureeParPiece, &couleur, &captured);
            board[i][j].contenu = contenu;
            board[i][j].captureeParPiece = captureeParPiece;
