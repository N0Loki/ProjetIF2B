...
void afficher_Pieces_restantes(int joueur, int totalPieces[3][7]) {
    ...
    printf("| %-12s | %2d |\n", "Tour ♖", totalPieces[joueur][2]);
    printf("| %-9s | %2d |\n", "Cavalier ♘", totalPieces[joueur][3]);
    printf("| %-12s | %2d |\n", "Fou ♗", totalPieces[joueur][4]);
...
TypePiece demanderPiece(int joueur, int totalPieces[3][7]) {
    ...
    while (1) {
        ...
        printf("\nQuelle pièce voulez-vous poser ? [pion,cavalier,fou,tour,reine,roi]\n");
        ...
        else if (strcmp(reponsePiece, "reine") == 0) piece = DAME;
